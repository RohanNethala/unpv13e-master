...
static void send_dv(router_t* R, const neighbor_t* nb){
    ...
    for (int i = 0; i < R->num_routes; i++) {
        ...
        msg.e[output_number].net = e->dest_net;
        msg.e[output_number].mask = e->mask;
        ...
        msg.e[output_number].cost = htons(send_cost);
        output_number++;
    }
    ...
    sendto(R->sock_ctrl, &msg, send_len, 0, (struct sockaddr*)&dest, sizeof(dest));
}
...
static bool dv_update(router_t* R, neighbor_t* nb, const dv_msg_t* m){
    ...
    for (uint16_t i = 0; i < message_number; i++) {
        ...
        // If we learned this route from this neighbor, update cost
        if (e->next_hop == nb->ip) {
            if (e->cost != new_cost) {
                e->cost = new_cost;
                e->last_update = time(NULL);
        ...
        // Otherwise, only update if this is a better path
        else if (new_cost < e->cost) {
            e->next_hop = nb->ip;
            e->cost = new_cost;
            e->last_update = time(NULL);
            changed = true;
    ...
    while(running){
        ...
        if(n > 0 && FD_ISSET(R.sock_ctrl, &rfds)){
            ...
            if (r > 0 && buf.type == MSG_DV) {
                ...
                // Fallback: match by port only (for loopback)
                if (!nb) {
                    for (int k = 0; k < R.num_neighbors; k++) {
                        if (R.neighbors[k].ctrl_port == src_port) {
                            nb = &R.neighbors[k];
                            break;
                        }
                    }
